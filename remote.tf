...
resource "null_resource" "set_passwords" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ES_VERSION=$(rpm -qa | grep '^elasticsearch-' | sed 's/elasticsearch-//' | cut -d'-' -f1 | cut -d'.' -f1)",
      "echo \"Detected Elasticsearch major version: $ES_VERSION\"",
      "if [ \"$ES_VERSION\" = \"8\" ]; then",
      "  echo 'Using Elasticsearch 8.x password reset tool...'",
      ...
      "  echo 'Using Elasticsearch 7.x password setup tool...'",
      "  echo 'Running elasticsearch-setup-passwords auto (will retry up to 20 times)...'",
      "  for i in {1..20}; do",
      "    if echo 'y' | sudo /usr/share/elasticsearch/bin/elasticsearch-setup-passwords auto > /tmp/password_setup_output.txt 2>&1; then",
      "      echo 'Passwords set successfully!'",
      "      cat /tmp/password_setup_output.txt",
      "      grep 'PASSWORD elastic = ' /tmp/password_setup_output.txt | awk '{print $NF}' > /tmp/elastic_password.txt",
      "      grep 'PASSWORD kibana_system = ' /tmp/password_setup_output.txt | awk '{print $NF}' > /tmp/kibana_password.txt",
      "      break",
      "    else",
      "      echo \"Retry $i/20 for password setup (cluster may still be initializing)...\"",
      "      sleep 20",
      "    fi",
      "  done",
      "else",
      "  echo 'ERROR: Unsupported Elasticsearch version'",
      "  exit 1",
      "fi",
      "echo 'Password setup complete!'",
      "echo 'Verifying password files...'",
      "if [ -s /tmp/elastic_password.txt ]; then echo 'Elastic password file OK'; else echo 'WARNING: Elastic password file is empty or missing'; fi",
      "if [ -s /tmp/kibana_password.txt ]; then echo 'Kibana password file OK'; else echo 'WARNING: Kibana password file is empty or missing'; fi"
    ]
  }
}
